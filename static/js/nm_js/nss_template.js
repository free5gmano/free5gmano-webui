...
function allocate_nssi(nsstID) {
    window.location.href = '/nssi_topology/?id='+nsstID+'&status=allocate';
}


function vnf_selected(url) {
    vnf_id = document.getElementById("vnf_template").value;
    if (vnf_id){
        axios.get(url + 'ObjectManagement/GenericTemplate/' + vnf_id + '/').then((response) => {
            vnf_description = response.data.description;
            document.getElementById("vnf_template_div").innerHTML = '<div>VNF ID: '+vnf_id+'<br>VNF Description: '+vnf_description+'</div>';
        }).catch((error) => {});
    }else{
        document.getElementById("vnf_template_div").innerHTML = '<div></div>';
    }
}


function nsd_selected(url) {
    nsd_id = document.getElementById("nsd_template").value;
    if (nsd_id){
        axios.get(url + 'ObjectManagement/GenericTemplate/' + nsd_id + '/').then((response) => {
            nsd_description = response.data.description;
            document.getElementById("nsd_template_div").innerHTML = '<div>NSD ID: '+nsd_id+'<br>NSD Description: '+nsd_description+'</div>';
        }).catch((error) => {});
    }else{
        document.getElementById("nsd_template_div").innerHTML = '<div></div>';
    }
}


function nrm_selected(url) {
    nrm_id = document.getElementById("nrm_template").value;
    if (nrm_id){
        axios.get(url + 'ObjectManagement/GenericTemplate/' + nrm_id + '/').then((response) => {
            nrm_description = response.data.description;
            document.getElementById("nrm_template_div").innerHTML = '<div>NRM ID: '+nrm_id+'<br>NRM Description: '+nrm_description+'</div>';
        }).catch((error) => {});
    }else{
        document.getElementById("nrm_template_div").innerHTML = '<div></div>';
    }
}