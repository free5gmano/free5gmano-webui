const url = 'http://10.20.1.111:8080/ObjectManagement/NetworkSliceSubnet/*/?scope=[%27BASE_NTH_LEVEL%27,1]';
var employees;
function get_nssi(){
    axios.get(url).then((response) => {
        employees = response.data.attributeListOut;
        // console.log(employees);
        for (var i = 0; i < employees.length; i++){
            // employees.Head_ID = response.data.attributeListOut[i].nssiId;
            employees[i].ID = employees[i].nssiId;
            employees[i].Head_ID = -1;
            // console.log(employees[i].ID);
            // console.log(employees[i]);
            if (employees[i].nsInfo){
                strdata = employees[i].nsInfo.vnfInstance;
                // console.log(strdata);
                strdata_handle = strdata.replace(/'/g, '"').replace(/:[ ]*False/g, ":false").replace(/:[ ]*True/g, ":true").replace(/:[ ]*None/g, ":null");
                // strdata_handle = strdata.replace(/'/g, '"').replace(/:[ ]*False/, ":false").replace(/:[ ]*True/, ":true").replace(/:[ ]*None/, ":none");
                result = JSON.parse(strdata_handle);
                employees[i].nsInfo = result;
            }
        }
        // for (var i = 0; i <= response.data.length; i++){
        //     employees.append("ID", i);
        //     employees.append("Head_ID", i);
        //     employees.append("Name", response.data[i].nssiId)
        // }
        console.log(employees);
      }); 
}
